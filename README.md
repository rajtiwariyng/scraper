# Laptop Data Scraper Tool

A comprehensive Laravel PHP application for scraping laptop product data from multiple e-commerce platforms including Amazon India, Flipkart, VijaySales, Reliance Digital, and Croma. The application features automated scheduling, database management, and a monitoring dashboard.
...
### 📊 Data Collected
For each laptop product, the scraper collects:
...
### 🎯 Supported Platforms
1. **Amazon India** - Comprehensive laptop catalog
2. **Flipkart** - Wide range of laptop products
...
crontab -e
# Add this line:
* * * * * cd /path/to/laptop-scraper && php artisan schedule:run >> /dev/null 2>&1
```
...
```apache
<VirtualHost *:80>
    DocumentRoot /path/to/laptop-scraper/public
    ServerName laptop-scraper.local
    
    <Directory /path/to/laptop-scraper/public>
        AllowOverride All
        Require all granted
    </Directory>
</VirtualHost>
...
server {
    ...
    root /path/to/laptop-scraper/public;
    index index.php;
...
CREATE INDEX idx_platform_active ON laptops(platform, is_active);
CREATE INDEX idx_brand_active ON laptops(brand, is_active);
CREATE INDEX idx_last_scraped ON laptops(last_scraped_at);
```
